...
st.markdown("<center><h1> Genera l' ACII ART con l' I.A.<small><br> Powered by INTELLIGENZAARTIFICIALEITALIA.NET </small></h1>", unsafe_allow_html=True)
st.write('<p style="text-align: center;font-size:15px;" > <bold>Trasforma qualsiasi immagine in ASCII ART con la nostra rete Neurale<bold>  </bold><p><br>', unsafe_allow_html=True)
...
option = st.selectbox(
     'Colore Primario',
     ('black', 'blue', 'pink','orange','red','green'))
...
def asciiart(in_f, SC, GCF,  out_f, color1, color2, bgcolor='white'):

    # The array of ascii symbols from white to black
    chars = np.asarray(list(' .,:irs?@9B&#-_@€$£%}{[]"'))
...
def imgGen2(img1):
  ...
  asciiart(inputf, SC, GCF, "results.png",option,option2)
